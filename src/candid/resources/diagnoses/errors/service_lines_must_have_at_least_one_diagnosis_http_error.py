# This file was auto-generated by Fern from our API Definition.

import typing

from ....core.api_error import ApiError
from ..types.service_lines_must_have_at_least_one_diagnosis_error import ServiceLinesMustHaveAtLeastOneDiagnosisError


class ServiceLinesMustHaveAtLeastOneDiagnosisHttpError(ApiError):
    def __init__(
        self, body: ServiceLinesMustHaveAtLeastOneDiagnosisError, headers: typing.Optional[typing.Dict[str, str]] = None
    ):
        super().__init__(status_code=422, headers=headers, body=body)
